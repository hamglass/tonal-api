...
# Paths — TONAL_TOKEN_DIR env var overrides for symlink/remote setups
...
def cmd_check_health(args):
    ...
    tokens = load_tokens()
    if not tokens:
        return {"health": "DEAD", "fix": "Re-authenticate with: tonal_tool.py auth <email> <password>"}
    expired = is_token_expired(tokens)
    ...
    if expired:
        ...
        result["fix"] = "Token expired. Try running any command (auto-refresh). If that fails, re-authenticate."
...
def cmd_create_workout(args):
    ...
    title = spec.get("title", "Custom Workout")
